...
def call(Map config = [:]) {
    ...
    if (!config.namespace) {
        error("❌ namespace is required! Please provide the K8s namespace (e.g., 'staging').")
    }
    if (!config.userServiceDeploymentName) {
        error("❌ userServiceDeploymentName is required! Please provide the deployment name.")
    }
    if (!config.todoServiceDeploymentName) {
        error("❌ todoServiceDeploymentName is required! Please provide the deployment name.")
    }
    ...
    def userServiceDeployment = config.userServiceDeploymentName
    def todoServiceDeployment = config.todoServiceDeploymentName
